...
import {
    ...
    Bell, Save, RefreshCw, AlertTriangle, Check, Coins
} from 'lucide-react';
...
const SystemSettings = () => {
    ...
    useEffect(() => {
        ...
        // Real-time subscription to prevent freeze
        ...
        const pollInterval = setInterval(fetchSettings, 60000);
    ...
    const handleSave = async (key, value) => {
        ...
        try {
            ...
            if (upsertError) {
                console.warn(`DB error saving ${key}:`, upsertError.message);
        ...
        <div className="p-10 max-w-[1600px] mx-auto space-y-10">
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                <SettingItem icon={Globe} title="Site Name" description="The public identifier for your platform">
                    <div className="flex gap-2">
                        ...
                        />
                        <button onClick={() => handleSave('site_name', settings.site_name)} className="p-3 bg-slate-900 text-white rounded-xl">
                            <Check size={20} />
                        </button>
                    </div>
                </SettingItem>
                ...
                {/* Daily Coins */}
                <SettingItem icon={Coins} title="Daily Coin Reward" description="Base coins users receive each day (manage streaks in Coin Rewards page)">
                ...
                {/* Maintenance Mode */}
                <SettingItem icon={Shield} title="Maintenance Mode" description="Prevent all non-admin users from accessing the platform">
                    <div className="flex items-center gap-4">
                        ...
                        <span className={`text-xs font-black uppercase tracking-widest ${settings.maintenance_mode ? 'text-red-500' : 'text-slate-400'}`}>
                            {settings.maintenance_mode ? 'ON – Site locked' : 'OFF – Site open'}
                        </span>
                    </div>
                    {settings.maintenance_mode && (
                        <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-xl">
                            <p className="text-xs font-black text-red-700">⚠️ Website is currently showing maintenance page to all non-admin users.</p>
                        </div>
                    )}
                </SettingItem>

                {/* Premium Priority Matching */}
                <SettingItem icon={Zap} title="Premium Priority Matching" description="Give premium users faster queue matching">
                    <button
                        onClick={() => {
            ...
            <div className="p-10 bg-indigo-600 rounded-[40px] text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl shadow-indigo-500/20 overflow-hidden relative">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-indigo-200 text-xs font-bold mt-2 uppercase tracking-widest">
                        📋 Manage subscription plans → Subscription Plans page &nbsp;|&nbsp; 💰 Coin rewards → Coin Rewards page
                    </p>
                </div>
